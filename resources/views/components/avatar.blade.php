...
<div class="relative"
  ...
  @if ($url)
    <x-link :href="url">
      ...
      @else
        <img src="{{ $data['content'] }}" alt="avatar" {{ $attributes->merge(['class' => 'rounded cursor-pointer']) }} />
      @endif
    </x-link>
  @else
    <div>
  ...
  <div class="absolute left-1/2 top-0 z-30 mt-5 w-[365px] max-w-lg -translate-x-1/2 translate-y-3" x-show="hoverCardHovered" x-cloak>
    <div class="flex h-auto w-[full] items-start space-x-3 rounded-md border border-neutral-200/70 bg-white p-5 shadow-sm" x-show="hoverCardHovered" x-transition>
      @if ($data['type'] === 'svg')
        <div class="h-20 w-20 rounded-full">
          {!! $data['content'] !!}
        </div>
      @else
        <img class="h-20 w-20 rounded-full" src="{{ $data['content'] }}" alt="avatar" />
      @endif
