    ...
    <nav class="flex py-3 text-gray-700">
      <ol class="inline-flex items-center space-x-1 md:space-x-3">
        ...
        <li>
          <div class="flex items-center">
            <x-heroicon-s-chevron-right class="mr-2 h-4 w-4 text-gray-400" />
            <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('Manage roles') }}</span>
          </div>
        </li>
      </ol>
    </nav>
        ...
        <ul class="w-full">
          @foreach ($view['roles'] as $role)
            <li class="group flex items-center justify-between px-6 py-4 hover:bg-slate-100 last:hover:rounded-b-lg">
              ...
              <!-- menu -->
              <ul>
                <li class="mr-2 inline"><x-link class="text-sm" href="{{ route('settings.role.edit', ['role' => $role['id']]) }}">{{ __('Edit') }}</x-link>
                </li>
                <li class="inline"><x-link class="text-sm" href="{{ route('settings.role.delete', ['role' => $role['id']]) }}">{{ __('Delete') }}</x-link>
                </li>
              </ul>
            </li>
          @endforeach
