<x-app-layout>
      ...
      <nav class="flex py-3 text-gray-700">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
          ...
          <li>
            <div class="flex items-center">
              <x-heroicon-s-chevron-right class="mr-2 h-4 w-4 text-gray-400" />
              <x-link class="text-sm" href="{{ route('settings.user.index') }}" wire:navigate>{{ __('Manage users') }}</x-link>
            </div>
          </li>
          <li>
            <div class="flex items-center">
  ...
  <div class="pb-12">
    <div class="mx-auto max-w-lg overflow-hidden rounded-lg bg-white shadow-md dark:bg-gray-800">
      <form method="POST" action="{{ $view['url']['update'] }}">
        @method('PUT')
        @csrf
        ...
        <div class="relative border-b px-6 py-4">
          <div class="relative mx-auto mb-4 h-32 w-32 overflow-hidden rounded-full">
            <x-avatar class="w-32" :data="$view['avatar']" />
          </div>
        ...
        <div class="relative px-6 py-4">
          <x-input-label class="mb-3" for="last_name" :value="__('What permissions should this person have?')" />
          ...
          <div class="space-y-2">
            <div class="flex items-center gap-x-2">
              ...
              <label class="block text-sm font-medium leading-6 text-gray-900" for="account_manager">{{ __('Account manager') }}</label>
            </div>
            <div class="flex items-center gap-x-2">
              <input class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-600"
                     id="administrator"
              ...
              <label class="block text-sm font-medium leading-6 text-gray-900" for="administrator">{{ __('Administrator') }}</label>
            </div>
            <div class="flex items-center gap-x-2">
              <input class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-600"
                     id="user"
              ...
              <label class="block text-sm font-medium leading-6 text-gray-900" for="user">{{ __('User') }}</label>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between border-t bg-gray-50 px-6 py-4">
          <x-link href="{{ route('settings.user.index') }}" wire:navigate>{{ __('Back') }}</x-link>
