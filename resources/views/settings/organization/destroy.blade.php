<x-app-layout>
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
          ...
          <li>
            <div class="flex items-center">
              <x-heroicon-s-chevron-right class="mr-2 h-4 w-4 text-gray-400" />
              <span
                    class="ml-1 text-sm text-gray-500 dark:text-gray-400 md:ml-2">{{ __('Delete the organization') }}</span>
            </div>
          </li>
        </ol>
      </nav>
  ...
  <div class="pb-12">
    <div class="mx-auto max-w-lg overflow-hidden rounded-lg bg-white shadow-md dark:bg-gray-800">
      <form method="POST"
            action="{{ route('settings.organization.destroy') }}">
        ...
        <div class="flex items-center justify-between bg-gray-50 px-6 py-4">
          <x-link wire:navigate href="{{ route('settings.index') }}">{{ __('Back') }}</x-link>
