<div class="rounded-lg bg-white shadow">
  ...
  <div class="{{ !$collapsed ? 'border-b' : '' }} flex items-center justify-between px-4 py-2">
    ...
    @else
      <div>
        @if ($context === 'message')
          <p>{{ __('Tasks') }}</p>
        @else
          <x-link class="text-blue-700 underline hover:rounded-sm hover:bg-blue-700 hover:text-white" href="{{ $taskList['parent']['url'] }}" wire:navigate>
    ...
    <div class="flex items-center">
      ...
      <div class="flex items-center">
        <!-- button -->
        <p class="mr-2 cursor-pointer rounded-lg border border-dashed border-gray-300 bg-gray-50 px-3 py-1 text-sm hover:border-gray-400 hover:bg-gray-200" wire:click="toggleAddModal()">
          {{ __('Add task') }}
        </p>

        <!-- collapse toggle -->
        @if ($collapsed)
          <div class="cursor-pointer rounded-lg px-1 py-1.5 text-gray-400 hover:bg-gray-100" wire:click="toggle()">
        ...
        <!-- option if the task list is not linked to a message -->
        @if ($taskList['parent']['is_project'])
          <div class="relative" x-data="{ dropdownOpen: false }">
            <button class="inline-flex items-center justify-center rounded-md border-gray-100 py-2 pl-3 text-sm font-medium text-neutral-700 transition-colors hover:border-gray-400 hover:bg-white focus:outline-none disabled:pointer-events-none disabled:opacity-50" @click="dropdownOpen=true">
            ...
            <div class="absolute left-1/2 top-0 z-50 mt-8 w-56 -translate-x-1/2"
              ...
              <div class="mt-1 rounded-md border border-neutral-200/70 bg-white p-1 text-neutral-700 shadow-md">
                <a class="relative flex cursor-pointer select-none items-center rounded px-2 py-1.5 text-sm outline-none transition-colors hover:bg-neutral-100 data-[disabled]:pointer-events-none data-[disabled]:opacity-50" href="{{ route('project.tasklist.edit', ['project' => $taskList['parent']['id'], 'tasklist' => $taskList['id']]) }}" wire:navigate.hover>
                  <x-heroicon-o-pencil class="mr-2 h-4 w-4" />
                  <span>{{ __('Edit') }}</span>
                </a>
                <a class="relative flex cursor-pointer select-none items-center rounded px-2 py-1.5 text-sm outline-none transition-colors hover:bg-neutral-100 data-[disabled]:pointer-events-none data-[disabled]:opacity-50" href="{{ route('project.tasklist.delete', ['project' => $taskList['parent']['id'], 'tasklist' => $taskList['id']]) }}" wire:navigate.hover>
  ...
  @if (!$collapsed)
    <div class="rounded-b-lg bg-gray-50">
                  ...
                  @if (count($task['assignees']) > 0)
                    <div class="flex -space-x-3">
                      @foreach ($task['assignees'] as $assignee)
                        <x-avatar class="h-6 w-6 cursor-pointer rounded-full border-2 border-white dark:border-gray-800" :data="$assignee['avatar']" />
                      @endforeach
                    </div>
                  @endif
      ...
      @if ($showAddTaskModal)
        <div class="px-4 py-2">
          <form class="flex items-center justify-between" wire:submit="save()">
            ...
            <div class="flex items-center">
              ...
              <span class="flex cursor-pointer rounded-md border border-gray-300 bg-gray-100 px-3 py-1 font-semibold text-gray-700 hover:border-solid hover:border-gray-500 hover:bg-gray-200" wire:click="toggleAddModal()">
                {{ __('Cancel') }}
              </span>
            </div>
          </form>
