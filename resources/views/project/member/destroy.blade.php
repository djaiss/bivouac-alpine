<x-app-layout>
          ...
          <li>
            <div class="flex items-center">
              ...
              <x-link class="text-sm"
                      ...
                      wire:navigate>{{ __('All the projects') }}</x-link>
            </div>
          </li>
          <li>
            <div class="flex items-center">
              <x-heroicon-s-chevron-right class="mr-2 h-4 w-4 text-gray-400" />
              <x-link class="text-sm"
                      href="{{ route('project.member.index', ['project' => $header['id']]) }}"
                      wire:navigate>{{ __('Manage project members') }}</x-link>
            </div>
          </li>
          <li>
            <div class="flex items-center">
              <x-heroicon-s-chevron-right class="mr-2 h-4 w-4 text-gray-400" />
              <span
                    class="ml-1 text-sm text-gray-500 dark:text-gray-400 md:ml-2">{{ __('Remove the member from the project') }}</span>
            </div>
          </li>
        </ol>
      </nav>
  ...
  <div class="pb-12">
    <div class="mx-auto max-w-lg overflow-hidden rounded-lg bg-white shadow-md dark:bg-gray-800">
      <form method="POST"
            action="{{ route('project.member.destroy', ['project' => $header['id'], 'user' => $view['id']]) }}">
        @csrf
        @method('delete')

        <div class="relative border-b px-6 py-4">
          <div class="h-3w-32 relative mx-auto mb-4 w-32 overflow-hidden rounded-full">
            <x-avatar class="mx-auto block rounded text-center"
                      :data="$view['avatar']" />
          </div>
        ...
        <div class="flex items-center justify-between bg-gray-50 px-6 py-4">
          <x-link href="{{ route('project.member.index', ['project' => $header['id']]) }}"
