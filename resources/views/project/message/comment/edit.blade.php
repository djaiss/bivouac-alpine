<x-app-layout>
  ...
  <div class="pb-12">
    <div class="mx-auto max-w-6xl overflow-hidden">
      <form class="grid grid-cols-[2fr_1fr] gap-4 px-4" method="POST" action="{{ route('project.message.comment.update', ['project' => $header['id'], 'message' => $view['message_id'], 'comment' => $view['id']]) }}">
        @csrf
        @method('PUT')
        ...
        <div>
          <div class="rounded-lg shadow">
            <div class="flex items-center justify-between rounded-t-lg border-b bg-white px-6 py-4">
              <x-link class="text-sm font-medium text-blue-700 underline hover:rounded-sm hover:bg-blue-700 hover:text-white" href="{{ route('project.message.show', ['project' => $header['id'], 'message' => $view['message_id']]) }}" wire:navigate>
                {{ __('Back') }}
              </x-link>
