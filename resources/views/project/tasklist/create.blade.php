<x-app-layout>
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
          ...
          <li>
            <div class="flex items-center">
              <x-heroicon-s-chevron-right class="mr-2 h-4 w-4 text-gray-400" />
              <span class="ml-1 text-sm text-gray-500 dark:text-gray-400 md:ml-2">{{ __('Add a task list') }}</span>
            </div>
          </li>
        </ol>
      </nav>
  ...
  <div class="pb-12">
    <div class="mx-auto max-w-lg overflow-hidden rounded-lg bg-white shadow-md dark:bg-gray-800">
      <form method="POST"
            action="{{ route('project.tasklist.store', ['project' => $header['id']]) }}">
        @csrf

        <div class="relative border-b px-6 py-4">
          <h1 class="text-center text-lg font-bold">{{ __('Create a task list') }}</h1>
        </div>

        <!-- Title -->
        <div class="relative px-6 py-4">
          <x-input-label for="name"
                         :value="__('What is the name of the list?')" />
        ...
        <div class="flex items-center justify-between border-t bg-gray-50 px-6 py-4">
          <x-link :href="route('project.tasklist.index', ['project' => $header['id']])"
                  wire:navigate.hover>{{ __('Back') }}</x-link>
