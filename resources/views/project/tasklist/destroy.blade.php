        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
          ...
          <li>
            <div class="flex items-center">
              <x-heroicon-s-chevron-right class="mr-2 h-4 w-4 text-gray-400" />
              <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('Delete the task list') }}</span>
            </div>
          </li>
        </ol>
      </nav>
      ...
      <form method="POST" action="{{ route('project.tasklist.destroy', ['project' => $header['id'], 'tasklist' => $view['id']]) }}">
        ...
        <div class="relative border-b px-6 py-4">
          ...
          <h1 class="text-center text-lg font-bold">
            {{ __('Delete the task list') }}
          </h1>
        </div>

        <div class="relative border-b px-6 py-4">
          <p>
            {{ __('Are you certain? This cannot be recovered. All tasks will be removed too.') }}
          </p>
        </div>
